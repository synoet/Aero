...
export class FlightController {
  ...
  createFlight = async (req: express.Request, res: express.Response) => {
    ...
    arrivalDate.setHours(
      arrivalDate.getHours() + Math.floor(Math.random() * 18) + randHours
    );
  ...
  getFlightSearchWithDateRange = async (
    req: express.Request,
  ...
  getFlightsView = async (req: express.Request, res: express.Response) => {
    ...
    let upcomingResponse = await Flight.find({})
      .limit(100)
      .sort({ departure_date: "desc" })
      .exec();
    let upcomingFlights = upcomingResponse
      .filter((flight) => flight.departure_date > currentDate)
      .slice(0, 4);
  ...
  getFlightByAirline = async (req: express.Request, res: express.Response) => {
    ...
    const flights = await Flight.find({ airline_namee: airline })
      .limit(100)
      .sort({ departure_date: "desc" })
      .exec();
