...
export class UserController {
    ...
    spending = async(req: express.Request, res: express.Response) => {
        ...
        if (user){
            const transactions = await Transaction.find({customer_email: user.email});
            await Promise.all(transactions.map(async (transaction: any) => {
                const purchase: any = await PurchaseInfo.findOne({transaction_id: transaction._id});
        ...
        res.status(200).send({totalSpending: total, spendingByMonths: spendingByMonths});
    }
    ...
    revenue = async(req: express.Request, res: express.Response) => {
        ...
        if(user.type === 'agent'){
            ...
            res.status(200).send({revenueSpending: total, revenueByMonths: revenueByMonths});
            ...
            await Promise.all(transactions.map(async (transaction: any) => {
                if (transaction.customer_email === transaction.booking_agent_email || transaction.booking_agent_email === null){
                    const purchase: any = await PurchaseInfo.findOne({transaction_id: transaction._id});
                    const month: any = new Date(purchase.purchase_date).getMonth();
                    const ticket: any = await Ticket.findOne({_id: purchase.ticket_id});
            ...
            res.status(200).send({totalRevenue: total, revenueByMonths: revenueByMonths});
        }
    }
